...
function SoundManager(smURL, smID) {
  ...
  this.flash9Options = {onfailure: null, isMovieStar: null, usePeakData: false, useWaveformData: false, useEQData: false, onbufferchange: null, ondataerror: null};
  ...
  this.ok = function() { return (needsFlash ? (didInit && !disabled) : (sm2.useHTML5Audio && sm2.hasHTML5)); };
  this.supported = this.ok;
  this.getMovie = function(movie_id) { return id(movie_id) || doc[movie_id] || window[movie_id]; };
    ...
    if(!resetEvents) {
      for(i in on_queue) {
        if(on_queue.hasOwnProperty(i)) {
			for(j = 0, k = on_queue[i].length; j < k; j++) on_queue[i][j].fired = false;
		}
      }
    } else {
      on_queue = [];
  ...
  this.getMoviePercent = function() { return (flash && 'PercentLoaded' in flash ? flash.PercentLoaded() : null); };
  ...
  SMSound = function(oOptions) {
    ...
    this.play = function(options, _updatePlayState) {
      ...
      if(!s.loaded) {
        if(s.readyState === 0) {
		  ...
		  playStatus = 'stopped';
		  currPlayerStatus('stopped');
          return s;
        } else {
          sm2._wD(fN + 'Loading - attempting to play...');
		  playStatus = 'connecting';
		  currPlayerStatus('loading');
        }
      } else {
      ...
      if(s.paused && s.position >= 0 && (!s._iO.serverURL || s.position > 0)) {
        ...
        if(s._iO.playbackRate !== 1) s.setPlaybackRate(s._iO.playbackRate);
        if(!s.isHTML5) {
    ...
    this.setPlaybackRate = function(playbackRate) {
      ...
      if(normalizedRate !== playbackRate) sm2._wD(s.id + ': setPlaybackRate(' + playbackRate + '): limiting rate to ' + normalizedRate, 2);
      if(s.isHTML5) {
        try {
    ...
    this.pause = function(_bCallFlash) {
      ...
      s.paused = true;
      if(!s.isHTML5) {
        if(_bCallFlash || _bCallFlash === _undefined) flash._pause(s.id, s._iO.multiShot);
      } else {
    ...
    attachOnPosition = function() {
      ...
      if(op) {
        for(item in op) if(op.hasOwnProperty(item)) s.onPosition(parseInt(item, 10), op[item]);
      }
    };
    detachOnPosition = function() {
      var item, op = s._iO.onposition;
      if(op) {
        for(item in op) if(op.hasOwnProperty(item)) s.clearOnPosition(parseInt(item, 10));
      }
    };
    ...
    add_html5_events = function() {
      ...
      var f;
      function add(oEvt, oFn, bCapture) { return s._a ? s._a.addEventListener(oEvt, oFn, bCapture || false) : null; }
    ...
    this._onload = function(nSuccess) {
      ...
      s._onbufferchange(0);
      if(!loadOK && !s.isHTML5) s._onerror();
      if(s._iO.onload) wrapCallback(s, function() { s._iO.onload.apply(s, [loadOK]); });
      return true;
    };
    this._onerror = function(errorCode, description) {
      if(s._iO.onerror) wrapCallback(s, function() { s._iO.onerror.apply(s, [errorCode, description]); });
    };
  ...
  function preferFlashCheck(kind) { return (sm2.preferFlash && hasFlash && !sm2.ignoreFlash && (sm2.flash[kind] !== _undefined && sm2.flash[kind])); }
  event = (function() {
    var old = (window.attachEvent), evt = {add: (old ? 'attachEvent' : 'addEventListener'), remove: (old ? 'detachEvent' : 'removeEventListener')};
  ...
  html5_events = {
    ...
    playing: html5_event(function() {
      ...
      this._s._onbufferchange(0);
	  playStatus = 'playing';
	  currPlayerStatus('playing');
	  ctInterval = setInterval(function() {
		  currTime+=21;
		  setWatch(currTime);
		}, 21);
    }),
  ...
  flashBlockHandler = function() {
    var name = str('fbHandler'), p = sm2.getMoviePercent(), css = swfCSS, error = {type: 'FLASHBLOCK'};
    if(sm2.html5Only) return;
    if(!sm2.ok()) {
      if(needsFlash) {
  ...
  processOnEvents = function(oOptions) {
    ...
    var status = {success: (oOptions && oOptions.ignoreInit ? sm2.ok() : !disabled)}, srcQueue = (oOptions && oOptions.type ? on_queue[oOptions.type] || [] : []), queue = [], i, j, args = [status], canRetry = (needsFlash && !sm2.ok());
  ...
  detectFlash = function() {
    ...
    if(nP && nP.length) {
      ...
      types = n.mimeTypes;
      if(types && types[type] && types[type].enabledPlugin && types[type].enabledPlugin.description) hasPlugin = true;
  ...
  badSafariFix = function() {
    if(!isBadSafari || !detectFlash()) return;
  ...
  createMovie = function(movieID, movieURL) {
    ...
    oEmbed = {name: movieID, id: movieID, src: movieURL, quality: 'high', allowScriptAccess: sm2.allowScriptAccess, bgcolor: sm2.bgColor, pluginspage: http + 'www.macromedia.com/go/getflashplayer', title: swfTitle, type: 'application/x-shockwave-flash', wmode: sm2.wmode, hasPriority: 'true'};
